...
public abstract class DonkeyMultiDBBaseTest {
	...
	public void tearDown() throws Exception {
		...
		for (CreateTableScriptEntry entry : createdTableList) {
			...
			Statement stmt = null;
			try {
				conn = DriverManager.getConnection(getDBBaseUrl()
						+ entry.getDbName() + ";DB_CLOSE_DELAY=-1");
				stmt = conn.createStatement();
				int count = 0;
				for (Map.Entry<String, String> table : entry
						.getTableNameScriptMapping().entrySet()) {
	...
	private List<DBDataEntry> parseDataFile() throws Exception {
		...
		Document configDoc = builder.parse(DonkeyMultiDBBaseTest.class
				.getClassLoader().getResourceAsStream(getDataFile()));
		XPathFactory xpathFactory = XPathFactory.newInstance();
		XPath xpath = xpathFactory.newXPath();
		NodeList databaseList = (NodeList) xpath.compile("/dataset/database")
	...
	private void parseCreateScriptConfigFile() throws Exception {
		...
		XPathFactory xpathFactory = XPathFactory.newInstance();
		XPath xpath = xpathFactory.newXPath();
		NodeList databaseList = (NodeList) xpath.compile("/databases/database")
				.evaluate(configDoc, XPathConstants.NODESET);
		for (int i = 0; i < databaseList.getLength(); i++) {
			CreateTableScriptEntry entry = new CreateTableScriptEntry();
			Element ele = (Element) databaseList.item(i);
			entry.setDbName(ele.getAttribute("name"));
			NodeList tableList = (NodeList) xpath.compile("tables/table")
					.evaluate(ele, XPathConstants.NODESET);
			Map<String, String> map = new HashMap<String, String>();
			for (int j = 0; j < tableList.getLength(); j++) {
				Element tableEle = (Element) tableList.item(j);
				map.put(tableEle.getAttribute("name"), tableEle
						.getTextContent());
			}
	...
	private static class CreateTableScriptEntry {
		 ...
		 */
		public void setTableNameScriptMapping(
				Map<String, String> tableNameScriptMapping) {
